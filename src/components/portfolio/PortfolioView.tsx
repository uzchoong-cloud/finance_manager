...
import { db, addStockHolding, buyShares, sellShares, deleteStockHolding } from "@/lib/db";
...
export function PortfolioView() {
  ...
  const { totalValue, totalCostBasis, totalGainLoss, totalGainLossPercent, holdings } =
    getPortfolioSummary();

  // Live query to keep UI in sync
  useLiveQuery(() => db.stockHoldings.toArray(), []);
  ...
  const openBuy = (holding: StockHolding) => {
    resetForm();
  ...
  const openSell = (holding: StockHolding) => {
    resetForm();
    setSelectedHolding(holding);
    setDialogMode("sell");
    setOpen(true);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!parsedShares || parsedShares <= 0 || !parsedPrice || parsedPrice <= 0) {
      toast.error("Enter valid shares and price");
      return;
    }
    setSaving(true);
    try {
      if (dialogMode === "add") {
        if (!ticker.trim() || !name.trim()) {
          toast.error("Enter ticker and company name");
          return;
        }
  ...
  const dialogTitle =
    dialogMode === "add"
      ? "Add New Position"
      : dialogMode === "buy"
      ? `Buy More ${selectedHolding?.ticker}`
      : `Sell ${selectedHolding?.ticker}`;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1
            style={{ fontSize: "1.75rem", fontWeight: 300, letterSpacing: "-0.5px", fontFeatureSettings: '"ss01"', color: "#061b31", lineHeight: 1.2 }}
          >
            Portfolio
          </h1>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <StatCard
          ...
          value={totalValue === null ? "Fetching…" : formatCurrencyCompact(totalValue)}
          subValue={totalValue !== null ? formatCurrency(totalValue) : undefined}
          trend="neutral"
        />
        <StatCard
          label="Cost Basis"
          value={formatCurrencyCompact(totalCostBasis)}
          subValue={formatCurrency(totalCostBasis)}
          trend="neutral"
        />
        <StatCard
          label="Total Return"
          value={
            totalGainLoss === null
              ? "—"
              : `${totalGainLoss >= 0 ? "+" : ""}${formatCurrencyCompact(totalGainLoss)}`
          }
          subValue={totalGainLossPercent !== null ? formatPercent(totalGainLossPercent) : undefined}
          trend={
            totalGainLoss === null ? "neutral" : totalGainLoss >= 0 ? "up" : "down"
          }
        />
      </div>
      ...
      {holdings.length === 0 ? (
        <div className="py-16 text-center rounded-lg" style={{ border: "1px dashed #b9b9f9" }}>
          <p style={{ fontSize: "14px", color: "#64748d", fontFeatureSettings: '"ss01"', fontWeight: 300 }}>
            No positions yet.{" "}
            <button onClick={openAdd} style={{ color: "#533afd", background: "none", border: "none", cursor: "pointer", fontFeatureSettings: '"ss01"' }}>
        ...
        <div className="space-y-3">
          {holdings.map((h) => {
            ...
            const gainColor =
              h.gainLoss === null ? "#64748d" : h.gainLoss >= 0 ? "#108c3d" : "#ea2261";
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="text-right shrink-0">
                    {isLoading ? (
                      <div className="h-5 w-20 rounded animate-pulse" style={{ background: "#f0f4f8" }} />
                    ) : isError ? (
                      <span style={{ fontSize: "12px", color: "#ea2261" }}>Price unavailable</span>
                    ) : h.currentValue !== null ? (
                      <>
                        <p
                          style={{
                        ...
                        <p style={{ fontSize: "12px", fontFeatureSettings: '"tnum"', color: gainColor }}>
                          {h.gainLoss !== null && h.gainLoss >= 0 ? "+" : ""}
                          {h.gainLoss !== null ? formatCurrency(h.gainLoss) : ""}
                ...
                <div className="mt-3 pt-3 flex items-center justify-between" style={{ borderTop: "1px solid #e5edf5" }}>
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => openBuy(h)}
                      style={{ fontSize: "11px", color: "#533afd", background: "rgba(83,58,253,0.06)", border: "1px solid rgba(83,58,253,0.2)", borderRadius: "4px", padding: "3px 8px", cursor: "pointer", fontFeatureSettings: '"ss01"' }}
                    >
                      Buy
                    </button>
                    <button
                      onClick={() => openSell(h)}
                      style={{ fontSize: "11px", color: "#273951", background: "rgba(39,57,81,0.06)", border: "1px solid rgba(39,57,81,0.2)", borderRadius: "4px", padding: "3px 8px", cursor: "pointer", fontFeatureSettings: '"ss01"' }}
                    >
                      Sell
                    </button>
                    <button
                      onClick={() => h.id && handleDelete(h.id, h.ticker)}
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent style={{ borderRadius: "8px", border: "1px solid #e5edf5", maxWidth: 440 }}>
          <DialogHeader>
            <DialogTitle
              style={{ fontSize: "1.125rem", fontWeight: 300, color: "#061b31", fontFeatureSettings: '"ss01"', letterSpacing: "-0.2px" }}
            >
              {dialogTitle}
            </DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-4 pt-2">
              ...
              <>
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    required
                    style={{ borderRadius: "4px", border: "1px solid #e5edf5", fontSize: "14px", fontFeatureSettings: '"tnum"', textTransform: "uppercase" }}
                  />
                </div>
                <div className="space-y-1">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <Input
                  ...
                  required
                  style={{ borderRadius: "4px", border: "1px solid #e5edf5", fontSize: "14px", fontFeatureSettings: '"tnum"' }}
                />
              </div>
              <div className="space-y-1">
            ...
            <div className="space-y-1">
              <Label style={{ fontFeatureSettings: '"ss01"', color: "#273951", fontSize: "13px" }}>Transaction Date</Label>
              <Input
                type="date"
                value={date}
            ...
            {dialogMode !== "add" && (
              <div className="space-y-1">
                ...
                <Input
                  placeholder="e.g. Q1 earnings buy"
                  value={notes}
            ...
            {shares && price && (
              <div
                ...
                style={{ background: "rgba(83,58,253,0.04)", border: "1px solid rgba(83,58,253,0.15)", borderRadius: "4px", fontFeatureSettings: '"tnum"', color: "#273951" }}
              >
